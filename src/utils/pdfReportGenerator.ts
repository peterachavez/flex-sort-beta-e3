...
export class PDFReportGenerator {
  ...
  private addHeader(data: ReportData) {
    ...
    this.pdf.text('Flex Sort Cognitive Assessment Report', this.margin, this.currentY + 10);
  ...
  private addSummaryOverview(data: ReportData) {
    ...
    this.pdf.text(data.assessmentData.cognitive_flexibility_score.toString(), this.margin + 5, this.currentY + 10);
    
    this.pdf.setFontSize(10);
    this.pdf.setTextColor(75, 85, 99);
    this.pdf.text('Cognitive Flexibility Score', this.margin + 5, this.currentY + 16);
    this.pdf.text(this.getScoreLabel(data.assessmentData.cognitive_flexibility_score), this.margin + 5, this.currentY + 21);

    // Rule Adaptations box
  ...
  public async generateReport(assessmentData: AssessmentData): Promise<void> {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      this.pdf.text(
        `Flex Sort Cognitive Assessment Report - Page ${i} of ${pageCount}`,
        this.margin,
        this.pageHeight - 10
      );
      this.pdf.text(
    ...
    const filename = `FlexSort_Report_${date}.pdf`;
